...
importlib.reload(Marche_BE)

from Exceptions import SplitError
from Exceptions import LoginError
from Exceptions import MarcheError
from UserAPP import Autocompletecombox

from Log_in.LoginU import LoginFrame
#----------------------------------------------------------------- PlaceHolder ---------------------------------------------------------------------------
...
class FrameWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.title("EpGis")
        self.geometry("485x280+400+220")
        self.resizable(0, 0)

        filepath = os.path.join(os.path.join(os.path.expanduser('~')), 'PycharmProjects\EpGis2\icons\logo.png')
        #self.iconbitmap(r'C:\Users\LAILA\PycharmProjects\IFE_GIS\icons\iconlogo.ico')
        #self.tk.call('wm', 'iconphoto', self._w, tk.PhotoImage(file=filepath))

        #self.iconphoto(False, tk.PhotoImage(file=filepath))
        ...
        self.frames = {}
        for F in (ConnectDB, Modifypass, LogIn):
...
class ConnectDB(tk.Frame):
    def __init__(self, parent, controller):
        ...
        lbl = Label(self, text="Veuillez Sélectionner Un Marché", font=("Helvetica", 15), bg="#F7F9F9")
        lbl.place(x=110, y=30)
        marche = Marche_BE.Marche()
        self.mrchVal = marche.dbCombbx()
        self.marche_var = StringVar()
        self.marche_box = Autocompletecombox.Autocompletecombox(self, textvariable=self.marche_var, width=44, font=("Helvetica", 13))
        self.marche_box.set_completion_list(self.mrchVal)
        self.marche_box.place(x=32, y=110)

        frm3 = Frame(self, bg="#379BF3", width=408, height=28)
        frm3.place(x=38, y=180)
        self.btn1 = tk.Button(frm3, text="Suivant", bg="#CEE6F3", font=("Times New Roman", 13), width=44, fg="#379BF3",
    ...
    def connect(self):
        try:
            ...
            dbVal = mrch.verifyDB(self.marche_box.get())
            if dbVal == True:
                self.controller.show_frame("LogIn")
                mrch.connect_marche(self.marche_box.get())
                #app1 = LoginFrame()
                #app1.mainloop()
            else:
                raise MarcheError.MarcheError
        except MarcheError.MarcheError:
            msg = messagebox.showerror("Choisir un Marché", "Le marché que vous avez choisi n'exsiste pas")
...
class LogIn(tk.Frame):
    ...
    def connexion(self):
        ...
        self.userentry.insert(0, "admin admin")
...
class Modifypass(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        add_placeholder_to(self.prnm_ent, 'Nom')
        ...
        add_placeholder_to(self.nm_ent, 'Prénom')
    ...
    def updat(self):
        ...
        if(self.psw_ent.get() != "Mot de passe" and self.psw_ent.get().isspace() == False and self.psw_ent.get() != ''):
            try:
                usr = User_BE.User()
            ...
            msg = messagebox.showwarning("Modifier votre mot de passe", "Mote de passe invalide")
