...
class Addspeculation(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        frm2 = Frame(self, bg="#379BF3", width=170, height=28)
        frm2.place(x=6, y=10)
        filterframe = Button(frm2, text="Nouveau Type de Spéculation", bg="#CEE6F3", fg="#379BF3", relief='flat', activebackground="#CEE6F3", activeforeground="#379BF3", command=self.add)
        filterframe.place(x=1, y=1)
        self.specult = TypeSpecl_BE.TypeSpecul()
        ...
        col_name = self.specult.columnName()
...
# -------------------------------------------------------------AFFICHAGE DE LIGNE SELECTIONNEE-------------------------------------------------
    def OnDoubleClick(self, event):
        try:
            ...
            verf = self.specult.verifyTypeSpeculation(str(value))
            if verf == True:
                self.root = tk.Tk()
                self.root.title("Type de spéculation")
                self.root.resizable(0, 0)
                self.root.geometry("370x230+450+190")
                self.root.config(bg="#F7F9F9")
                self.frameDoubleClick()
            elif verf == False:
                showerror("Type de spéculation", "Veuillez sélectionner type de spéculation")
        except:
            showerror("Type de spéculation", "Veuillez sélectionner type de spéculation")
    ...
    def frameDoubleClick(self):
        ...
        self.row_selected = self.tree.item(self.tree.selection())
        value = self.row_selected['values'][0]
        colonnes = self.specult.select_colName()
        valeurs = self.specult.select_colValue(str(value))
    ...
    def modification(self):
        if self.h[0].get() != '':
            ...
            self.specult.update_colValue(tab, str(self.row_selected['values'][0]))
            ...
            showerror("Spéculation", "Veuillez remplir le champ: Libelle Fr")
    ...
    def suppr(self):
        ...
        label = tk.Label(self.popinfos, text="Voulez-vous supprimer ce type de spéculation?", font=("Helvetica", 12), pady=13, padx=20, bg="#F7F9F9").place(x=12, y=12)
    ...
    def supprimer(self):
        self.specult.deleteTypeSpeculation(str(self.row_selected['values'][0]))
        self.root.destroy()
...
#-----------------------------------------------------------L'ajout d'un Nouveau Type de Speculation-----------------------------------------------------
    ...
    def add(self):
        ...
        label = LabelFrame(self.root, text="Nouveau type de spéculation", font=("Helvetica", 13), width=380, height=230)
        label.place(x=10, y=8)

        label = LabelFrame(self.root, text="Ajouter un type de spéculation", font=("Helvetica", 13), bg="#F7F9F9", width=380, height=230)
        label.place(x=10, y=8)
        ...
        spBtn = Button(self.root, text="Ajouter", font=("Times New Roman", 11), bg="#4EB1FA", fg="#FFFFFF", relief=FLAT, activebackground="#4EB1FA", activeforeground="#000000", command=self.ajouter)
        spBtn.place(x=170, y=190)

    def ajouter(self):
        if self.libelleFr_entry.get() != '':

            self.specult.insertTypeSpeculation(self.libelleFr_entry.get(), self.libelleAr_entry.get(), self.abrv_entry.get())
            self.tree.delete(*self.tree.get_children())
