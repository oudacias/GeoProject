...
from Synchronisation import Gnr_Server, FromServ_Server
...
# SELECT Machine
    ...
    def sendData(self):
        if self.machinBox.get()!= '' and self.tpSynchBox.get()!= '':
            ...
            if tp == 'du jour':
                synchJr = FromServ_Server.SynchServerJr()
                synchJr.sendData(self.machinBox.get())
            elif tp == 'générale':
                synchGnr = Gnr_Server.SynchServerGnr()
                synchGnr.sendData(self.machinBox.get())
        else:
